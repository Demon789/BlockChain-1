...
binariesInstall() {
  ...
  binaryDownload ${BINARY_FILE} https://github.com/hyperledger/fabric/releases/${ARCH}-${VERSION}/${BINARY_FILE}
  if [ $? -eq 22 ]; then
     echo
  ...
  binaryDownload ${CA_BINARY_FILE} https://github.com/hyperledger/fabric/releases/${ARCH}-${CA_VERSION}/${CA_BINARY_FILE}
  if [ $? -eq 22 ]; then
     echo
